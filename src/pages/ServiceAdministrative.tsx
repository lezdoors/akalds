...
export default function ServiceAdministrative() {
  const { t } = useTranslation();
  const { getLocalizedPath } = useLanguage();
  const seoConfig = getPageSEO('administrative');
  ...
  const features = [0, 1, 2, 3].map((i) => t(`services.items.administrative.features.${i}`));

  const steps = [0, 1, 2, 3].map((i) => ({
    step: t(`services.items.administrative.process.${i}.step`),
    description: t(`services.items.administrative.process.${i}.description`),
  }));

  const faqItems = [0, 1, 2].map((i) => ({
    question: t(`services.items.administrative.faq.${i}.question`),
    answer: t(`services.items.administrative.faq.${i}.answer`),
  }));
      ...
      <main>
        ...
        <section className="section-padding pt-32 pb-20" ref={heroRef}>
          <div className="container-custom text-center max-w-4xl mx-auto">
            ...
            >
              {t('services.items.administrative.title')}
            </motion.h1>
            <motion.p
            ...
            >
              {t('services.items.administrative.description')}
            </motion.p>
          </div>
        </section>
        ...
        {/* Process timeline */}
        <ServiceProcess steps={steps} title={t('services.items.administrative.title')} />
