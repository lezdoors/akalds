...
export function PortfolioStrip() {
  ...
  return (
    <section className="border-t border-stone-200 bg-[#f5f4ed] py-20 lg:py-24 text-stone-900">
          ...
          <div className="grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-stone-200 bg-stone-200 md:grid-cols-3">
            {ITEMS.map((item, idx) => (
              <BlurFade key={item.key} delay={0.15 + idx * 0.1} inView>
                <div className="group flex h-full flex-col gap-4 bg-white p-8 md:p-10 transition-colors hover:bg-[#f5f4ed]">
                  <item.Icon
